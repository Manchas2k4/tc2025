...
void un_consumidor() {
	...
	while (k-- > 0) {
		i = (rand() % 5) + 1;
		printf("consumidor %i entrando al almacen\n", getpid());
		sem_wait(semid, LLENO, i);
		
		printf("consumidor %i intentando retirar %i producto(s)\n", getpid(), i);
		mutex_wait(semid, MUTEX);
		printf("consumidor %i va a retirar los productos\n", getpid());
		for (j = 0; j < i; j++) {
			b->data[b->next++] = (rand() % 3) + 1;
			printf("consumidor %i retirando el numero %i\n", getpid(), b->data[b->next - 1]);
		}
		printf("buffer = ");
		for (j = 0; j < SIZE; j++) {
			printf("%3i", b->data[j]);
		...
		printf("\n");
		printf("consumidor %i retiro sus productos\n", getpid());
		mutex_signal(semid, MUTEX);
		
		sem_signal(semid, VACIO, i);
